...
class Textarea extends Field {
	...
	public function render() {
		$template = '<div id="%1$s" class="%2$s"><label for="%3$s">%4$s</label><p><textarea id="%3$s" name="%5$s" placeholder="%6$s" class="%7$s" %8$s %9$s %10$s %11$s>%12$s</textarea></p>%13$s</div>';
		...
		return sprintf(
			...
			$this->get_name(),                      // %5$s: Textarea name
			...
			esc_textarea( $this->get_value() ),     // %12$s: Textarea value
			$this->get_description( 'p' )           // %13$s: Description
		);
	}
}
