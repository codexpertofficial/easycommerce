...
class Text extends Field {
	...
	public function render() {
		$template = '<div id="%1$s" class="%2$s"><label for="%3$s">%4$s</label><p><input type="%5$s" id="%6$s" name="%7$s" value="%8$s" placeholder="%9$s" class="%10$s" %11$s %12$s %13$s %14$s /></p>%15$s</div>';
		...
		return sprintf(
			...
			$this->get_name(),                      // %7$s: Input name
			esc_attr( $this->get_value() ),         // %8$s: Input value
			esc_attr( $this->get_placeholder() ),   // %9$s: Placeholder text
			esc_attr( $this->get_field_class() ),   // %10$s: Input class
			$this->is_disabled() ? 'disabled' : '', // %11$s: Disabled attribute
			$this->is_readonly() ? 'readonly' : '', // %12$s: Readonly attribute
			$this->is_required() ? 'required' : '', // %13$s: Required attribute
			$this->render_atts(),                   // %14$s: Rendered additional attributes
			$this->get_description( 'p' )           // %15$s: Description
		);
	}
}
