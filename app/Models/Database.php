...
class Database {
	...
	public function set_table( $table_name ) {
 	 ...
 	 *
 	 * @return string $primary_key The primary key for the table.
 	 */
	 ...
	 */
	public function get_rows( $conditions = array(), $limit = 0, $offset = 0, $order = 'DESC', $orderby = 'id' ) {

		$clause = $this->prepare_clause( $conditions );
		$where_clause = $clause['where'];
		$values       = $clause['values'];

		$where_clause_str = ! empty( $where_clause ) ? 'WHERE ' . implode( ' AND ', $where_clause ) : '';

		// Validate $order against an explicit allowlist to prevent injection.
		$order = in_array( strtoupper( (string) $order ), array( 'ASC', 'DESC' ), true ) ? strtoupper( $order ) : 'DESC';

		// Validate $orderby is a safe SQL identifier (alphanumeric + underscore only).
		if ( ! preg_match( '/^[a-zA-Z0-9_]+$/', $orderby ) ) {
			$orderby = $this->primary_key;
		}

		$order_clause = 'DESC' === $order ? " ORDER BY $orderby DESC" : " ORDER BY $orderby ASC";
	...
	/**
	 * Prepares an SQL clause
	 * 
	 * @see get_rows()
	 * @return [ 'where' => 'where clause statements', 'values' => 'clause values' ]
	 */
	private function prepare_clause( $conditions ) {
		$where_clause = array();
		$values       = array();
		...
		return [ 'where' => $where_clause, 'values' => $values ];
	}
	...
	public function exec( $sql, $output = OBJECT ) {
