...
const ChangeStatusTop = `${EASYCOMMERCE.assets}admin/img/icons/ChangeStatusTop.png`;
const arrowDown = `${EASYCOMMERCE.assets}admin/img/icons/arrowDown.png`;

const options = Object.keys(EASYCOMMERCE.order_statuses).map((key) => {
    return {
        value: key,
        label: EASYCOMMERCE.order_statuses[key],
    };
});

const ChangeStatus = ({ hideModal, order, updateOrder }) => {
    const dispatch = useDispatch();
    const [isOpen, setIsOpen] = useState(false);
    const [label, setLabel] = useState(
        options.find((option) => option.value === order.status).label
    );
    ...
    const handleChangeStatus = () => {
        ...
        fetch(
            `${EASYCOMMERCE.rest_base}/orders/${order.id}?status=${selectedStatus}`,
            {
                method: "PUT",
                headers: {
            ...
            .then((data) => {
                ...
                if (data.success && data.data?.order_id) {
                    ...
                    updateOrder({ ...order, status: selectedStatus });
                    hideModal();
                } else {
                    dispatch(
                        addToastData({
                            ...
                            <div className="mb-4 flex flex-col">
                                ...
                                <div className="relative flex items-center justify-center">
                                    <button
                                        ...
                                        style={{
                                            boxShadow: isOpen
                                                ? "0px 2px 4.5px 0px var(--color-ec-secondary)"
                                                : "",
                                        }}
                                    >
