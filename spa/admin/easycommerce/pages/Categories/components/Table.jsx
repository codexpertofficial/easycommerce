...
const Table = ({ categories, fetchData, onEdit , isLoading, bulkDeleteIds, setBulkDeleteIds }) => {
    const [showModal, setShowModal] = useState(false);
    const [categoryIdToDelete, setCategoryIdToDelete] = useState(null);

    const handleDelete = async () => {
        try {
            const response = await fetch(`${EASYCOMMERCE.rest_base}/products/categories/${categoryIdToDelete}`, {
                method: 'DELETE',
                headers: {
        ...
        <>
                                        ...
                                        onChange={(e) => {
                                            if (e.target.checked) {
                                                categories.forEach(cat => {
                                                    setBulkDeleteIds(prev => [...prev, cat.id]);
                                                });
                                            } else {
                                                setBulkDeleteIds([]);
                                            }
                                ...
                                <tr key={category.id} className={`w-full text-sm font-normal`} style={{ height: '56px' }}>
                                    ...
                                    >
                                        <input 
                                            type="checkbox" 
                                            className='easycommerce-input-checkoutbox'
                                            checked={bulkDeleteIds.includes(category.id)}
                                            onChange={(e) => {
                                                if (e.target.checked) {
                                                    setBulkDeleteIds([...bulkDeleteIds, category.id]);
                                                } else {
                                                    setBulkDeleteIds(bulkDeleteIds.filter(id => id !== category.id));
                                                }
                                            }}
                                        />
                                    </td>
            ...
            {showModal && (
                <DeletePopup
                    ...
                    itemName={categories.find(attr => attr.id === categoryIdToDelete)?.name || ''}
                />
            )}
        </>
    );
