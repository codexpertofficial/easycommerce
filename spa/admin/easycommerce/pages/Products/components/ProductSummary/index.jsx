...
const ProductSummary = ({ productTitle, prevData }) => {
	const [isOpen, setIsOpen] = useState(true);
	const [aiOpen, setAiOpen] = useState(false);
	const [summary, setSummary] = useState(prevData || '');

	const handleSummaryChange = async (value) => {
		setSummary('');

		for (let i = 0; i < value.length; i++) {
			await new Promise((resolve) => setTimeout(resolve, 5));
			setSummary((prev) => prev + value[i]);
		}
	};
		...
		<>
			{aiOpen && (
				<AiGenerate
					...
					setAiOpen={setAiOpen}
					setAiContent={handleSummaryChange}
				/>
			)}
				...
				<div className="py-[14px] px-6 flex items-center justify-between border-b border-ec-table-stock border-solid">
					...
					<div className="panel-actions">
						...
						>   
							...
							>
								<path 
									d="M1.12891 4.28906L5.28516 0.378906C5.43099 0.251302 5.58594 0.1875 5.75 0.1875C5.91406 0.1875 6.0599 0.251302 6.1875 0.378906L10.3438 4.28906C10.6172 4.59896 10.6263 4.90885 10.3711 5.21875C10.0794 5.49219 9.76953 5.5013 9.44141 5.24609L5.75 1.74609L2.03125 5.24609C1.72135 5.5013 1.42057 5.5013 1.12891 5.24609C0.873698 4.91797 0.873698 4.59896 1.12891 4.28906Z" fill="#3C3C42"
								/>
							</svg>
						</button>
					</div>
