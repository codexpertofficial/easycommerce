...
// Usage is only registered as a tab once connected (see app/Config/settings.php);
// while disconnected the "Connectivity" tab carries the connect call-to-action.
// This guards direct-URL access to the usage submenu before connecting.
$ec_api       = get_option( 'easycommerce_api' );
$ec_connected = ! empty( $ec_api->email );

if ( ! $ec_connected ) {
	printf(
		'<div class="easycommerce-ai-usage"><p style="padding:24px;color:#7f7f98;font-size:14px;">%s</p></div>',
		esc_html__( 'Connect your store to the EasyCommerce AI service to see your AI credit usage.', 'easycommerce' )
	);
	return;
}
