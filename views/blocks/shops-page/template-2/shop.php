<?php
/**
 * Template for rendering product shop items in Template 2.
 *
 * This template displays individual product cards with images, titles, ratings, prices, etc. in a grid layout.
 *
 * @var array $args Template arguments.
 * @var array $args['products'] Array of formatted product data.
 * @var array $args['settings'] Block settings for styling and configuration.
 * @var string $args['shop_name'] Shop template name (e.g., 'template-2').
 * @var string $args['view'] View type for CSS classes (e.g., 'easycommerce-shop-container').
 */
...
foreach ( $products as $product ) :
	...
	?>
		<div class="col-span-1 border border-[#F7F7F8] rounded-xl easycommerce-single-product">
			<div class="w-full flex items-center justify-center rounded-t-xl overflow-hidden">
				...
				<a class="w-full inline-block overflow-hidden" href="<?php echo esc_attr( $product['link'] ); ?>"
					...
					<div class="relative">
						...
						<img src="<?php echo esc_url( $image_url ); ?>" alt="<?php echo esc_attr( $product['title'] ); ?>"
							class="hover:scale-110 duration-500 transition border rounded-t-lg easycommerce-thumbnail-img" />
							<?php
						} else {
							?>
						<img src="<?php echo esc_url( EASYCOMMERCE_ASSETS_URL . 'public/img/product/shop-product-placeholder.png' ); ?>"
							alt="<?php echo esc_attr( $product['title'] ); ?>"
							class="border rounded-t-lg easycommerce-thumbnail-img" />
							<?php
						}
						?>
					</div>
			...
			<div class="p-4">
				...
				<div class="mb-2 font-inter" style="
							color: <?php echo esc_attr( $category_color ); ?>;
							font-size: <?php echo esc_attr( $category_fontsize ); ?>px;
							font-weight: <?php echo esc_attr( $category_fontweight ); ?>;
							text-transform: <?php echo esc_attr( $category_text_transform ); ?>;
							font-style: <?php echo esc_attr( $category_text_style ); ?>;
							text-decoration: <?php echo esc_attr( $category_decoration ); ?>;
							line-height: <?php echo esc_attr( $category_line_height ); ?>px;
							letter-spacing: <?php echo esc_attr( $category_letter_spacing ); ?>px;
						">
				...
				<?php endif; ?>
				<div class="easycommerce-product-price-wrap mt-3 flex flex-wrap gap-4 items-start">
					<div class="w-full flex items-center justify-between flex-wrap">
						<div class="flex items-center gap-x-3 flex-wrap font-inter">
							<?php
							do_action( 'easycommerce_before_product_price', $product );
						...
						<?php
						do_action( 'easycommerce_after_product_price', $product );
						?>
					</div>
					<?php
					...
					<button data-id="<?php echo esc_attr( $product['id'] ); ?>"
						class="<?php echo esc_attr( $ec_cart_btn_class ); ?> easycommerce-button-margin easycommerce-add-to-cart-shop<?php echo $is_out_of_stock ? ' opacity-50 cursor-not-allowed' : ''; ?>"
						...
						<span id="buttonText" class="add-to-cart-text">
							<?php esc_html_e( 'Add', 'easycommerce' ); ?>
						</span>
						<div id="loader" class="loader" style="display: none;"></div>
					</button>
					
					<!-- <div class="easycommerce-single-product-checkout-btn">
								<a href="<?php echo esc_url( get_permalink( easycommerce_cart_redirect() ) ); ?>" class="flex items-center gap-3 justify-center p-3 font-inter text-base leading-[26px] w-full border-none rounded-md text-ec-primary font-medium hover:!bg-ec-primary hover:!text-white transition-all ease-in-out duration-500">Proceed to checkout <img className="ml-3" height="10" width="26" src="<?php echo EASYCOMMERCE_ASSETS_URL . 'admin/img/icons/arrow-blue-right.png'; ?>">  </a>
							</div> -->
					...
					<a href="<?php echo esc_attr( $product['link'] ); ?>"
						class="<?php echo esc_attr( $ec_cart_btn_class ); ?> flex items-center gap-1 px-5 py-[7px] !no-underline easycommerce-shop-choose-btn rounded-md transition-all ease-in-out duration-500<?php echo $is_out_of_stock ? ' opacity-50 pointer-events-none' : ''; ?>"
					>
						<svg class="w-5 h-5" data-slot="icon" fill="none" stroke-width="1.5" stroke="currentColor"
							viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
							<path stroke-linecap="round" stroke-linejoin="round"
